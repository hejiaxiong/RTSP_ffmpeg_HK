...
void CALLBACK MessageCallback(LONG lCommand, NET_DVR_ALARMER *pAlarmer, char *pAlarmInfo, DWORD dwBufLen, void* pUser)
{
	...
	else
	{
		g_pMainDlg->AddLog(-1, ALARM_INFO_T, "alarm host get alarm[%x] from device[%s] port[%d] name[%s] mac:%x:%x:%x:%x:%x:%x serial:%s", \
            lCommand, pAlarmer->sDeviceIP, pAlarmer->wLinkPort, pAlarmer->sDeviceName, pAlarmer->byMacAddr[0], pAlarmer->byMacAddr[1], pAlarmer->byMacAddr[2], \
	...
	try
	{
		...
		else if (COMM_UPLOAD_FACESNAP_RESULT == lCommand)
		{
            ...
            ((LPNET_VCA_FACESNAP_RESULT)pAlarmMsg)->pUIDBuffer = new BYTE[((LPNET_VCA_FACESNAP_RESULT)pAlarmMsg)->byUIDLen];
            memset(((LPNET_VCA_FACESNAP_RESULT)pAlarmMsg)->pUIDBuffer, 0, ((LPNET_VCA_FACESNAP_RESULT)pAlarmMsg)->byUIDLen);
            memcpy(((LPNET_VCA_FACESNAP_RESULT)pAlarmMsg)->pUIDBuffer, ((LPNET_VCA_FACESNAP_RESULT)pAlarmInfo)->pUIDBuffer, ((LPNET_VCA_FACESNAP_RESULT)pAlarmMsg)->byUIDLen);
		...
		else if (COMM_ITS_PLATE_RESULT == lCommand)
		{
            ...
            if (((LPNET_ITS_PLATE_RESULT)pAlarmMsg)->struPlateInfo.dwXmlLen > 0)
            {
                ((LPNET_ITS_PLATE_RESULT)pAlarmMsg)->struPlateInfo.pXmlBuf =
                    (char*)pAlarmMsg + sizeof(NET_ITS_PLATE_RESULT) + dwPrePicLen + sizeof(NET_ITS_ILLEGAL_INFO);
            }
		}
...
void CClientDemoDlg::ProcRuleAlarm(WPARAM wParam, LPARAM lParam)
{
	...
	//行为分析结果上报结构
    sprintf(szInfoBuf, "Smart:[%d] DevIP:[%s]TargetID[%d]RuleID[%d]RuleName[%s]AbsTime[%d]piclen[%d]PicType[%d]RelAlarmPicNum[%d]AlarmID[%d]IvmsChannel[%d]IvmsChannelEx[%d]EventTypeEx[%d]byPicTransType[%d]",
        ...
        struVcaRuleAlarm.struRuleInfo.wEventTypeEx, struVcaRuleAlarm.byPicTransType);
    g_pMainDlg->AddLog(iDeviceIndex,ALARM_INFO_T,szInfoBuf);
...
void CClientDemoDlg::ProcTfsAlarm(WPARAM wParam,LPARAM lParam)
{
    ...
    sprintf(szInfoBuf, "Tfs Alarm IvmsChan[%d]ID[%d]Name[%s]Type[%d]Abs[%4.4d%2.2d%2.2d%2.2d%2.2d%2.2d]DevIP[%s]Port[%d]Chan[%d]SpecificVehicleType[%d]\
        ...
        PilotSunVisor[%d]CopilotSunVisor[%d]PilotCall[%d]IllegalCode[%s] VehicleEnterState[%d]Country[%d]Region[%d]",
...
void CClientDemoDlg::ProcFaceSnapAlarm(WPARAM wParam, LPARAM lParam)
{
    ...
    sprintf(szInfoBuf, "Face Snap alarm BrokenNetHttp[%d]Abs[%4.4d%2.2d%2.2d%2.2d%2.2d%2.2d] FaceID[%d] Smart[%d] AgeGroup[%d] Sex[%d] EyeGlass[%d] \
		StayDuration[%.3f] FaceScore[%d] AlarmEndMark[%d] RepeatTimes[%d]-StorageIP[%s] Storage Port[%d],IvmsChannel[%d]IvmsChannelEx[%d] \
        Age[%d], AgeDeviation[%d] FacePicQuality[%d] UIDLen[%d]\n",
    ...
    if (struFaceSnapAlarm.byUIDLen > 0 && struFaceSnapAlarm.pUIDBuffer != NULL)
    {
        ...
        memcpy(cUID, struFaceSnapAlarm.pUIDBuffer, struFaceSnapAlarm.byUIDLen - 1);
        ...
        DWORD dwReturn = 0;
        HANDLE hFile = NULL;
        sprintf(cFilenameTemp, "%s\\AlarmInfo[%s].txt", cFilename, chTime);
    ...
    if (struFaceSnapAlarm.pUIDBuffer != NULL)
    {
        delete[]struFaceSnapAlarm.pUIDBuffer;
        struFaceSnapAlarm.pUIDBuffer = NULL;
    }
}
...
// 			struFaceDetectionAlarm.struFacePic[i].fY != 0)
    ...
    sprintf(szInfoBuf, "[Detection Face Snap alarm] Abs[%4.4d%2.2d%2.2d%2.2d%2.2d%2.2d] RelativeTime[%4.4d%2.2d%2.2d%2.2d%2.2d%2.2d] Dev[ip:%s,port:%d,ivmsChan:%d] IvmsChannelEx[%d] UploadEventDataType[%d] \n", \
        struAbsTime.dwYear, struAbsTime.dwMonth, struAbsTime.dwDay, struAbsTime.dwHour,struAbsTime.dwMinute, struAbsTime.dwSecond, \
		struRelativeTime.dwYear, struRelativeTime.dwMonth, struRelativeTime.dwDay, struRelativeTime.dwHour,struRelativeTime.dwMinute, struRelativeTime.dwSecond, \
...
void CClientDemoDlg::ProcFaceSnapMatchAlarm(WPARAM wParam, LPARAM lParam)
{
    ...
    sprintf(szInfoBuf, "Face Snap Match alarm BrokenNetHttp[%d] Abs[%4.4d%2.2d%2.2d%2.2d%2.2d%2.2d] FaceID[%d] BlackList ID[%d] StorageIP[%s]-StoragePort[%d] Sex[%d] Glasses[%d] Age[%d] FacePicQuality[%d]\
        ...
        struFaceSnapMatchAlarm.struBlackListInfo.struBlackListInfo.byType, struFaceSnapMatchAlarm.byContrastStatus, struFaceSnapMatchAlarm.byModelingStatus, struFaceSnapMatchAlarm.byMask, struFaceSnapMatchAlarm.bySmile);
    AddLog(m_iCurDeviceIndex, ALARM_INFO_T, szInfoBuf);
...
void CClientDemoDlg::ProcITSPlateAlarm(WPARAM wParam, LPARAM lParam)
{
    ...
    sprintf(szInfoBuf, "ITS Plate Alarm PlateInfo PlateType[%d]Color[%d]Bright[%d]LicenseLen[%d]EntireBelieve[%d]Region[%d]Country[%d]Area[%d]PlateSize[%d]PlateCategory[%s]License[%s]PlateRect[x-%f y-%f w-%f h-%f] \
       VehicleInfo:Index[%d]VehicleType[%d] Color[%d] RadarState[%d] Speed[%d] VehicleLogoRecog[%d] VehicleSubLogoRecog[%d]", \
...
void CClientDemoDlg::ProcVideoIntercomEvent(WPARAM wParam,LPARAM lParam)
{
    ...
    {
        sprintf(szInfoBuf, "Video Intercom Event Time[%4.4d%2.2d%2.2d%2.2d%2.2d%2.2d] DevNumber[%s] EventType[%d], FloorNumber[%d], RoomNumber[%d], LockID[%d]",
            struVideoIntercomEvent.struTime.wYear, struVideoIntercomEvent.struTime.byMonth, struVideoIntercomEvent.struTime.byDay,
            struVideoIntercomEvent.struTime.byHour, struVideoIntercomEvent.struTime.byMinute, struVideoIntercomEvent.struTime.bySecond,
...
{  
	...
	{
		sprintf(szInfoBuf, "Video Intercom Alarm Time[%4.4d%2.2d%2.2d%2.2d%2.2d%2.2d] DevNumber[%s] AlarmType[%d] LockID[%d]", 
			struVideoIntercomAlarm.struTime.wYear, struVideoIntercomAlarm.struTime.byMonth, struVideoIntercomAlarm.struTime.byDay,
			struVideoIntercomAlarm.struTime.byHour, struVideoIntercomAlarm.struTime.byMinute, struVideoIntercomAlarm.struTime.bySecond, 
			struVideoIntercomAlarm.byDevNumber,struVideoIntercomAlarm.byAlarmType,
            struVideoIntercomAlarm.wLockID);
...
void CClientDemoDlg::ProThermometryAlarm(WPARAM wParam, LPARAM lParam)
{
    ...
    {
        sprintf(szInfoBuf, "%s: Channel:%d, RuleID:%d, ToleranceTemperature:%f, AlertFilteringTime:%d, AlarmFilteringTime:%d,ThermometryUnit:%d, PresetNo:%d, RuleTemperature:%.1f, CurrTemperature:%.1f, PTZ Info[Pan:%f, Tilt:%f, Zoom:%f], AlarmLevel:%d, \
                                                                  AlarmType:%d, AlarmRule:%d, RuleCalibType:%d,  Point[x:%f, y:%f], PicLen:%d, ThermalPicLen:%d, ThermalInfoLen:%d", szLan, \
                                                                  struThermometryAlarm.dwChannel, struThermometryAlarm.byRuleID, struThermometryAlarm.fToleranceTemperature, struThermometryAlarm.dwAlertFilteringTime, struThermometryAlarm.dwAlarmFilteringTime, struThermometryAlarm.byThermometryUnit, struThermometryAlarm.wPresetNo, \
    ...
    else if (1 == struThermometryAlarm.byRuleCalibType || 2 == struThermometryAlarm.byRuleCalibType)
    {
        ...
        sprintf(szInfoBuf, "%s: Channel:%d, RuleID:%d,ToleranceTemperature:%f, AlertFilteringTime:%d, AlarmFilteringTime:%d,HighestPoint[x:%f, y:%f],ThermometryUnit:%d, PresetNo:%d, RuleTemperature:%.1f, CurrTemperature:%.1f, PTZ Info[Pan:%f, Tilt:%f, Zoom:%f], AlarmLevel:%d, \
                                                                  AlarmType:%d, AlarmRule:%d, RuleCalibType:%d,  Region[%s], PicLen:%d, ThermalPicLen:%d, ThermalInfoLen:%d", szLan, \
                                                                  struThermometryAlarm.dwChannel, struThermometryAlarm.byRuleID, struThermometryAlarm.fToleranceTemperature, struThermometryAlarm.dwAlertFilteringTime, struThermometryAlarm.dwAlarmFilteringTime, struThermometryAlarm.struHighestPoint.fX, struThermometryAlarm.struHighestPoint.fY, struThermometryAlarm.byThermometryUnit, struThermometryAlarm.wPresetNo, \
                ...
                if (TargetList != NULL)
                {
                    ...
                    if (iCount > 0)
                    {
                        ...
                        if (Target != NULL)
                        {
                            ...
                            {
                                sprintf(szInfoBuf, "Json format alarm  recognitionType:[%s]", recognitionType->valuestring);
