 ...
 ****************************************************************************/
 class JDialogPlayBack extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        jRadioButtonByCardNumber.setBounds(90, 140, 70, 21);

        jTextFieldCardNumber.setText("0");
        jPanel1.add(jTextFieldCardNumber);
        jTextFieldCardNumber.setBounds(90, 170, 190, 21);

        jLabel14.setForeground(new java.awt.Color(153, 153, 153));
        jLabel14.setText("NVR(32路或者以下)的IP通道的通道号从33开始");
        jPanel1.add(jLabel14);
        jLabel14.setBounds(10, 120, 260, 15);
        ...
        jButtonSearch.setBounds(10, 350, 60, 23);
        ...
        jPanelTime.setBounds(10, 210, 270, 130);

        jLabel13.setText("卡号");
        jPanel1.add(jLabel13);
        jLabel13.setBounds(10, 170, 40, 15);

        jLabel20.setText("是否按卡号");
        jPanel1.add(jLabel20);
        jLabel20.setBounds(10, 140, 80, 15);

        jLabel21.setText("通道号");
        jPanel1.add(jLabel21);
        jLabel21.setBounds(10, 90, 36, 15);
        ...
        jToggleButtonExit.setBounds(650, 580, 60, 23);
        ...
        jButtonDownload.setBounds(520, 580, 60, 23);
        getContentPane().add(jProgressBar);
        jProgressBar.setBounds(10, 580, 410, 20);
    ...
    {//GEN-HEADEREND:event_jButtonDownloadActionPerformed
       //如果不在下载,开始下载
        if (m_lDownloadHandle.intValue() == -1)
        {
            ...
            //文件保存路径，查找到的文件名作为保存文件名
            String sSaveFileName = "c:/DownLoad/" + sFileName + ".mp4";
            System.out.println(sSaveFileName);

            m_lDownloadHandle = hCNetSDK.NET_DVR_GetFileByName(m_lUserID, sFileName, sSaveFileName);
            if (m_lDownloadHandle.intValue() >= 0)
            {
                if(iTransType > 0)
                {
                    IntByReference intTypeByRef = new IntByReference(iTransType);
                    Pointer lpInBufferType = intTypeByRef.getPointer();
                    if(!hCNetSDK.NET_DVR_PlayBackControl_V40(m_lDownloadHandle, HCNetSDK.NET_DVR_SET_TRANS_TYPE, lpInBufferType,4,null,null))
                    {
                        JOptionPane.showMessageDialog(this, "设置下载文件封装格式失败!错误号:" + hCNetSDK.NET_DVR_GetLastError());
                        return;
                    }
                    JOptionPane.showMessageDialog(this, "设置下载文件封装格式成功!类型值为" + iTransType);
                }

                IntByReference intByRef = new IntByReference(0);
		Pointer lpInBuffer = intByRef.getPointer();
                hCNetSDK.NET_DVR_PlayBackControl_V40(m_lDownloadHandle, HCNetSDK.NET_DVR_PLAYSTART, lpInBuffer,4,null,null);
